...
def sniffing_function(packet: scapy.Packet) -> None:
    ...
    if packet.haslayer(HTTPRequest):
        ...
        # Check if the request is classified as a threat
        if req.threats is not None and 'valid' not in req.threats:
            # Close the connection if the request is classified as a threat
            LOGGER.info(f"Force close connection from {packet[IP].src}:{packet[TCP].sport} to {packet[IP].dst}:{packet[TCP].dport}")
            force_close_connection(packet[IP].src, packet[TCP].sport, packet[IP].dst, packet[TCP].dport)
        else:
            LOGGER.info(f"Good connection from {packet[IP].src}:{packet[TCP].sport} to {packet[IP].dst}:{packet[TCP].dport}")
